...
import com.meerkatgramv2auth.global.errors.custom.business.InvalidTokenException;
import com.meerkatgramv2auth.global.errors.custom.business.NotRegisteredException;
...
public class AuthService {
  ...
  public AuthResponseDTO reissue(HttpServletRequest request, HttpServletResponse response) {
    ...
    User user = authRepository.findById(userId)
        .orElseThrow(() -> new InvalidTokenException("유효하지 않은 회원의 토큰"));

    // 비로그인 상태 확인
    if(user.getRefreshToken() == null) {
      throw new InvalidTokenException("비로그인 상태");
    }
